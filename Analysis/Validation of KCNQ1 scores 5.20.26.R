...
tf4=read.csv("Current scores/KCNQ1Scores_6.1.26.csv",stringsAsFactors = FALSE)

# If you have already run through the pipeline through STEP 4, load in tf5
tf5=read.csv("Current scores/KCNQ1Scores2_6.1.26.csv",stringsAsFactors = FALSE)
...
brnichCounting=function(tf5_control,scoreName,lowerCutoff,upperCutoff,missenseOnly=TRUE,results,invertDirection=FALSE,strictClinvar=FALSE){
  ...
  # ROC Math
  tf5_control=tf5_control[!is.na(tf5_control$controlFinal) & (tf5_control$controlFinal=='plp' | tf5_control$controlFinal=='blb'),]
  midpoint_df <- data.frame(
    assay = c("trafficking_score", "function_score", "het_trafficking_score", "het_function_score","GNB_prob","LR_prob","RF_prob"),
    midpoint = c(0.95, 0.9855, 1.0005, 1, 0, 0, 0)
  )
  midpoint=midpoint_df[midpoint_df$assay==scoreName,'midpoint']
  scores <- tf5_control[,scoreName]
  scores = abs(midpoint-scores) #YYYYY
  labels <- tf5_control$controlFinal
  roc_obj <- roc(labels, scores)
  roc_auc <- round(auc(roc_obj),2)
  
  results[currRow,'ROC_AUC']=roc_auc
  return(results)
}
...
curation2=curation2[,c('mutation','peakCurrentNorm','deltaV1.2act.Mut.WT.','hetPeakCurrentNorm','paper','cell.type')]
names(curation2)=c('mutation','peakCurrent_lit','deltaV12act_lit','het_PeakCurrent_lit','paper','cellType')
curation2$peakCurrent_lit=curation2$peakCurrent_lit/100
curation2$het_PeakCurrent_lit=curation2$het_PeakCurrent_lit/100
a2=a2[,c('mutation','Tail.CD.mean','deltaV12act_thisStudy','het_PeakCurrent_thisStudy','paper','cellType')]
names(a2)=names(curation2)
manualFunction=rbind(curation2,a2) #318 rows
manualFunction[manualFunction$cellType=='HEK (TRAF) & CHO (FUNC)','cellType']='CHO'
summary(as.factor(manualFunction$cellType))
#CHO                            COS7 Guinea Pig Ventricular Myocytes                          HEK293 
#221                               2                               1                              56 
#HL1                           mouse                         Xenopus 
#1                               4                              33 
...
# HEK
manualFunction_HEK=manualFunction[manualFunction$cellType=='HEK293',]
manualFunction2_HEK <- aggregate(peakCurrent_lit ~ mutation, data = manualFunction_HEK, FUN = function(x) mean(x, na.rm = TRUE))
manualFunction3_HEK <- aggregate(deltaV12act_lit ~ mutation, data = manualFunction_HEK, FUN = function(x) mean(x, na.rm = TRUE))
manualFunction4_HEK <- aggregate(het_PeakCurrent_lit ~ mutation, data = manualFunction_HEK, FUN = function(x) mean(x, na.rm = TRUE))
names(manualFunction2_HEK)[2]='peakCurrent_lit_HEK'
names(manualFunction3_HEK)[2]='deltaV12act_lit_HEK'
names(manualFunction4_HEK)[2]='het_PeakCurrent_lit_HEK'

# CHO only
manualFunction_CHO=manualFunction[manualFunction$cellType=='CHO',]
manualFunction2_CHO <- aggregate(peakCurrent_lit ~ mutation, data = manualFunction_CHO, FUN = function(x) mean(x, na.rm = TRUE))
manualFunction3_CHO <- aggregate(deltaV12act_lit ~ mutation, data = manualFunction_CHO, FUN = function(x) mean(x, na.rm = TRUE))
manualFunction4_CHO <- aggregate(het_PeakCurrent_lit ~ mutation, data = manualFunction_CHO, FUN = function(x) mean(x, na.rm = TRUE))
names(manualFunction2_CHO)[2]='peakCurrent_lit_CHO'
names(manualFunction3_CHO)[2]='deltaV12act_lit_CHO'
names(manualFunction4_CHO)[2]='het_PeakCurrent_lit_CHO'

# HEK or CHO only
manualFunction_HEKCHO=manualFunction[manualFunction$cellType=='CHO'|manualFunction$cellType=='HEK293',]
manualFunction2_HEKCHO <- aggregate(peakCurrent_lit ~ mutation, data = manualFunction_HEKCHO, FUN = function(x) mean(x, na.rm = TRUE))
manualFunction3_HEKCHO <- aggregate(deltaV12act_lit ~ mutation, data = manualFunction_HEKCHO, FUN = function(x) mean(x, na.rm = TRUE))
manualFunction4_HEKCHO <- aggregate(het_PeakCurrent_lit ~ mutation, data = manualFunction_HEKCHO, FUN = function(x) mean(x, na.rm = TRUE))
names(manualFunction2_HEKCHO)[2]='peakCurrent_lit_HEKCHO'
names(manualFunction3_HEKCHO)[2]='deltaV12act_lit_HEKCHO'
names(manualFunction4_HEKCHO)[2]='het_PeakCurrent_lit_HEKCHO'

# All automated patch clamp
manualFunction_AUTO=manualFunction[manualFunction$paper=='thisStudy'|manualFunction$paper=='30571187'|manualFunction$paper=='33095155'|manualFunction$paper=='34930020'|manualFunction$paper=='39969993',]
manualFunction2_AUTO <- aggregate(peakCurrent_lit ~ mutation, data = manualFunction_AUTO, FUN = function(x) mean(x, na.rm = TRUE))
manualFunction3_AUTO <- aggregate(deltaV12act_lit ~ mutation, data = manualFunction_AUTO, FUN = function(x) mean(x, na.rm = TRUE))
manualFunction4_AUTO <- aggregate(het_PeakCurrent_lit ~ mutation, data = manualFunction_AUTO, FUN = function(x) mean(x, na.rm = TRUE))
names(manualFunction2_AUTO)[2]='peakCurrent_lit_AUTO'
names(manualFunction3_AUTO)[2]='deltaV12act_lit_AUTO'
names(manualFunction4_AUTO)[2]='het_PeakCurrent_lit_AUTO'
...
manual_tables <- list(
  ...
  manualFunction3_CHO,
  manualFunction4_CHO,
  manualFunction2_HEKCHO,
  manualFunction3_HEKCHO,
  manualFunction4_HEKCHO,
  manualFunction2_AUTO,
  manualFunction3_AUTO,
  manualFunction4_AUTO
)
...
# Merge in gnomad, missense/nonsense/synon (NEW)
gnomad=read.csv('Validation/KCNQ1_gnomAD_v4.1.1_forR.csv',stringsAsFactors = FALSE)
duplicated_rows <- gnomad %>% group_by(mutation) %>% filter(n() > 1) %>% ungroup() #211 duplicated rows
...
# How many variants at drug binding positions have reduced abundance?
tf4_drugBindingT=tf4[drugBindingTF,]
tf4_drugBindingT$mutation
hist(tf4_drugBindingT$trafficking_score, breaks=30)
toKeep=tf4_drugBindingT$trafficking_score<.61
tf4_drugBindingT_low=tf4_drugBindingT[toKeep,]
dim(tf4_drugBindingT) #112
dim(tf4_drugBindingT_low) #55
# So 55 of 112 (49% of drug binding variants had abundance scores below 0.61 (reduced abundance)

# Counting number of Clinvar P/LP missense variants at drug binding positions
tf4_drugBindingT=tf4[drugBindingTF,]
dim(tf4_drugBindingT) #112
summary(as.factor(tf4_drugBindingT$clinvar))
...
write.csv(tf4,"Current scores/KCNQ1Scores_6.1.26.csv",row.names=FALSE)
...
write.csv(tf4_dn,"Current scores/KCNQ1Scores_noDrugBindingPos_forDNheatmap_6.1.26.csv",row.names=FALSE)
...
write.csv(tf5,"Current scores/KCNQ1Scores2_6.1.26.csv",row.names=FALSE)
...
# Plot of classifications vs MAVE scores for variants only from 239-307
tf5_control$midTF <- tf5_control$pos >= 239 & tf5_control$pos <= 307
tf5_control_path=tf5_control[tf5_control$controlFinal_missense=='plp',]
pdf('Validation/Clinvar_violin_midNonsense_Violin_6.2.26.pdf')
ggplot(tf5_control_path, aes(x = midTF, y = function_score, fill = midTF)) +
  geom_violin(scale='width') +
  geom_quasirandom(fill = 1, color = "black") +
  labs(x = 'Mid or not?', y = 'Function score', title = 'Clinvar comparison (function scores)') +
  theme_classic()+
  geom_hline(yintercept = 0.25, linetype = "dashed") +
  geom_hline(yintercept = 0.58, linetype = "dashed") +
  geom_hline(yintercept = 1.391, linetype = "dashed")
dev.off()
sum(tf5_control_path$midTF & tf5_control_path$function_score<0.58,na.rm=TRUE) #44
sum(tf5_control_path$midTF & tf5_control_path$function_score>=0.58,na.rm=TRUE) #8
# So 44/(44+8)=85% of the mid missense P/LP's have reduced MAVE function scores
sum(!tf5_control_path$midTF & tf5_control_path$function_score<0.58,na.rm=TRUE) #123
sum(!tf5_control_path$midTF & tf5_control_path$function_score>=0.58,na.rm=TRUE) #29
# So 123/(123+29)=81% of the non-mid missense P/LP's have reduced MAVE function scores
sum(tf5_control_path$function_score<0.58,na.rm=TRUE) #167
sum(tf5_control_path$function_score>=0.58,na.rm=TRUE) #37
# So 167/(167+37)=82% of all missense P/LP's have reduced MAVE function scores
...
results=brnichCounting(tf5_control,'trafficking_score',0.61,1.29,missenseOnly=TRUE,results,strictClinvar=FALSE)
results=brnichCounting(tf5_control,'function_score',0.58,1.391,missenseOnly=TRUE,results,strictClinvar=FALSE)
results=brnichCounting(tf5_control,'het_trafficking_score',0.676,1.325,missenseOnly=TRUE,results,strictClinvar=FALSE)
results=brnichCounting(tf5_control,'het_function_score',0.363,1.717,missenseOnly=TRUE,results,strictClinvar=FALSE)
# All variants including synon and nonsense; controlFinal
results=brnichCounting(tf5_control,'trafficking_score',0.61,1.29,missenseOnly=FALSE,results,strictClinvar=FALSE)
results=brnichCounting(tf5_control,'function_score',0.58,1.391,missenseOnly=FALSE,results,strictClinvar=FALSE)
results=brnichCounting(tf5_control,'het_trafficking_score',0.676,1.325,missenseOnly=FALSE,results,strictClinvar=FALSE)
results=brnichCounting(tf5_control,'het_function_score',0.363,1.717,missenseOnly=FALSE,results,strictClinvar=FALSE)
...
results
write.csv(results,'Validation/Brnich_summary_forExcel2.csv',row.names=FALSE)

# Brnich Summary #
results_preferred=results[results$missenseOnly & !results$strictClinvar,]
results_preferred[1:4,c(1,13:23)]
...
pdf('Validation/HomTrafficking_vs_HomFunction_raw_9.11.25_4x6.pdf', width = 4, height = 6)
#pdf('Validation/HomTrafficking_vs_HomFunction_raw_9.11.25_5x6.pdf', width = 5, height = 6)
#pdf('Validation/HomTrafficking_vs_HomFunction_raw_9.11.25_6x8.pdf', width = 6, height = 8)
#pdf('Validation/HomTrafficking_vs_HomFunction_raw_9.11.25_7x7.pdf', width = 7, height = 7)
...
# Plot all variants with B and P highlighted on top
plot(tf5_missense$trafficking_score, tf5_missense$function_score, pch=20, cex=0.4, col = "grey70", xlab='Hom trafficking score', ylab='Hom function score', main='Missense variants', xlim=c(-0.1, 1.55), ylim=c(-0.5, 3), xaxt = 'n')
axis(1, at = seq(-0.5, 1.5, by = 0.5)) # Custom x-axis labels
text(0.25, 2.75, "rho=0.57")
abline(v=0.25)
abline(v=0.61)
...
points(tf_P$trafficking_score, tf_P$function_score, col = "red",  pch = 20, cex=1.8)
points(tf_B$trafficking_score, tf_B$function_score, col = "blue", pch = 17, cex=1.8)
#mutation trafficking_score function_score
#A341V         1.17      0.16
#R366W         1.01      0.31
points(c(1.17,1.01),c(0.16,0.31),col='purple') #A341V and R366W
...
# How many of the 339 activity-sensitive mutants had a dominant negative effect in the function assay?
dim(gatingMutantDF) #339
hist(gatingMutantDF$het_function_score,breaks=50)
sum(!is.na(gatingMutantDF$het_function_score)) #319 have a het function score
sum(gatingMutantDF$het_function_score<0.363,na.rm=TRUE) #114 have a het function score below the cutoff
...
sum(!is.na(tf5$peakCurrent_lit)) #226 variants

# Hom Patch Clamp vs Hom DMS Function --HEK ONLY
cor.test(tf5$peakCurrent_lit_HEK, tf5$function_score, method = "spearman") #rho=0.57
sum(!is.na(tf5$peakCurrent_lit_HEK)) #41 variants

# Hom Patch Clamp vs Hom DMS Function --CHO ONLY
cor.test(tf5$peakCurrent_lit_CHO, tf5$function_score, method = "spearman") #rho=0.66
sum(!is.na(tf5$peakCurrent_lit_CHO)) #171 variants

# Hom Patch Clamp vs Hom DMS Function --HEK AND CHO ONLY
cor.test(tf5$peakCurrent_lit_HEKCHO, tf5$function_score, method = "spearman") #rho=0.65
sum(!is.na(tf5$peakCurrent_lit_HEKCHO)) #206 variants
# So the correlation improved a little bit to 0.65 when we restricted it to HEK and CHO cells only

# Hom Patch Clamp vs Hom DMS Function --AUTO ONLY (automated patch clamp)
cor.test(tf5$peakCurrent_lit_AUTO, tf5$function_score, method = "spearman") #rho=0.68
sum(!is.na(tf5$peakCurrent_lit_AUTO)) #174 variants
# So the correlation improved to 0.68 when we restricted it to automated patch clamp only

# Hom Patch Clamp vs Hom DMS Function --THIS PAPER ONLY
cor.test(tf5$peakCurrent_thisStudy, tf5$function_score, method = "spearman") #rho=0.80
sum(!is.na(tf5$peakCurrent_thisStudy)) #19 variants
# So the correlation was 0.80 when restricted to the variants studied on the SP in this study
...
ggplot(tf5, aes(x = peakCurrent_lit, y = function_score)) +
  ...
  ggtitle('Lit manual vs DMS Function scores (n=226 variants)') +
...
ggplot(tf5_missense_under4, aes(x = trafficking_lit, y = trafficking_score)) +
  ...
  geom_vline(xintercept = 1, color = "darkgrey") +
  stat_smooth(method = "loess", se = TRUE) +
  ggtitle('Manual vs DMS trafficking scores (n=158 variants)') +
...
tf5_missense=tf5[tf5$mut_type=='missense',]
plot(tf5_missense$function_score,
     log10(tf5_missense$AlleleFrequency),
     col = "black",
     pch = 1,
...
# High AF
highAF=tf5[tf5$AlleleFrequency>3*10^-5 & (tf5$mut_type=='missense' | tf5$mut_type=='nonsense'),] #36
sum(highAF$function_score<=1.391 & highAF$function_score>=0.58 & !is.na(highAF$function_score)) #35
#35/36 missense variants with high AF have normal function scores

# Which variants are GOF and in gnomAD?
tf5_gof2=tf5[tf5$cluster_n6_name_FINAL=="GOF" & !is.na(tf5$cluster_n6_name_FINAL) & (tf5$mut_type=='missense'|tf5$mut_type=='mid_nonsense'|tf5$mut_type=='late_nonsense'),] #126
summary(as.factor(tf5_gof2$mut_type))
tf5_gof2_gnomad=tf5_gof2[tf5_gof2$AlleleFrequency>0,] #6
summary(as.factor(tf5_gof2_gnomad$mut_type))
#late_nonsense      missense 
#1             6 
tf5_gof2_gnomad$mutation #"E663K" "E663Q" "L665M" "P660L" "P660S" "T661I" "Y662X"
tf5_gof2_gnomad=tf5_gof2_gnomad[order(tf5_gof2_gnomad$AlleleFrequency,decreasing=TRUE),]
tf5_gof2_gnomad[,c('mutation','AlleleFrequency')]
sum(tf5_gof2_gnomad$AlleleFrequency) #2.0e-4
sum(tf5_gof2_gnomad$AlleleFrequency[2:7]) #1.0e-5
# Of the 126 GOF missense or nonsense variants (GOF k-means cluster), only 19 were present in gnomAD, with a total allele frequency of 1.0e-5 (Table SX).
# Future work will be necessary to determine the phenotypic associations of these variants.
# Y662X is 20X more common than the remaining 6 missense variants.
...
##### STEP 32: Comparison with PE-ACMG Criteria #####
# Comparing to 34798354
pe1=read.csv('Validation/KCNQ1 PE-ACMG Mayo.csv',stringsAsFactors = FALSE) #79
pe2=read.csv('Validation/KCNQ1 PE-ACMG Amsterdam.csv',stringsAsFactors = FALSE) #42
head(pe1)
head(pe2)
...
pe_merged <- bind_rows(
  pe1 %>% mutate(source = "Mayo"),
  pe2 %>% mutate(source = "Amsterdam")
) %>%
  mutate(
    PE.Criteria.clean = ifelse(PE.Criteria == "_", "VUS", PE.Criteria),
    pe_rank = pe_rank(PE.Criteria.clean),
    pe_intensity = abs(pe_rank)
  ) %>%
  group_by(Amino.Acid.Variant) %>%
  arrange(
    desc(pe_intensity),  # farther from VUS wins
    desc(pe_rank),       # if same distance, pathogenic direction wins
    .by_group = TRUE
  ) %>%
  slice(1) %>%
  ungroup() %>%
  select(-pe_rank, -pe_intensity)

dim(pe_merged) #103
pe_merged <- as.data.frame(pe_merged)
names(pe_merged)[2]='mutation'
pe_merged2=merge(pe_merged,tf5[,c(1,10)],all.x=TRUE)

plot_df <- pe_merged2 %>%
  filter(!is.na(function_score), !is.na(PE.Criteria.clean)) %>%
  mutate(
    PE.Criteria.clean = ifelse(PE.Criteria.clean == "_", "VUS", PE.Criteria.clean),
    
    PE.Criteria.plot = ifelse(PE.Criteria.clean == "VUS", "none", PE.Criteria.clean),
    PE.Criteria.plot = factor(
      PE.Criteria.plot,
      levels = c("PS5", "PM7", "none", "BP8")
    )
  ) %>%
  ungroup() %>%
  as.data.frame()

pdf('Validation/KCNQ1 PE-ACMG violin.pdf')
ggplot(plot_df, aes(x = PE.Criteria.plot, y = function_score, fill = PE.Criteria.plot)) +
  geom_violin(scale = "width") +
  geom_quasirandom(fill = 1, color = "black") +
  labs(
    x = "Proposed PE-ACMG criterion",
    y = "Function score",
    title = "Proposed PE-ACMG comparison (function scores)"
  ) +
  theme_classic() +
  geom_hline(yintercept = 0.25, linetype = "dashed") +
  geom_hline(yintercept = 0.58, linetype = "dashed") +
...
pe_merged3=pe_merged2[!is.na(pe_merged2$function_score),]
summary(as.factor(pe_merged3$PE.Criteria))
#_ BP8 PM7 PS5 
#6   6  19  67 
sum(pe_merged3$PE.Criteria=='PS5' & pe_merged3$function_score<0.58) #57
sum((pe_merged3$PE.Criteria=='PM7'|pe_merged3$PE.Criteria=='BP8'|pe_merged3$PE.Criteria=='_') & pe_merged3$function_score<0.58) #57



# COMPARING TWO FILES:
tf5_new=read.csv("Current scores/KCNQ1Scores2_6.1.26.csv",stringsAsFactors=FALSE) #13403x64
tf5_old=read.csv("Current scores/KCNQ1Scores2_9.11.25.csv",stringsAsFactors=FALSE) #13403x52

# 1. Compare column names
new_cols <- names(tf5_new)
old_cols <- names(tf5_old)

cols_only_new <- setdiff(new_cols, old_cols)
cols_only_old <- setdiff(old_cols, new_cols)
cols_shared   <- intersect(old_cols, new_cols)
...
cat("\nNumber of shared columns:", length(cols_shared), "\n")

# 2. Check whether row counts are the same
cat("\nRows in new:", nrow(tf5_new), "\n")
cat("Rows in old:", nrow(tf5_old), "\n")

# 3. Restrict to shared columns, using old column order
new_shared <- tf5_new[, names(tf5_old), drop = FALSE]
old_shared <- tf5_old[, names(tf5_old), drop = FALSE]

# 4. Check exact equality on shared columns
cat("\nExact equality on shared columns:\n")
print(identical(new_shared, old_shared))

# 5. If not identical, summarize differences by column
diff_summary <- data.frame(
  column = names(old_shared),
  n_different = sapply(names(old_shared), function(col) {
    sum(!(new_shared[[col]] == old_shared[[col]] | 
            (is.na(new_shared[[col]]) & is.na(old_shared[[col]]))))
  }),
  stringsAsFactors = FALSE
)

diff_summary <- diff_summary[diff_summary$n_different > 0, ]

cat("\nColumns with differences:\n")
print(diff_summary)

# 6. Get row-level differences
different_rows <- which(!apply(
  mapply(function(a, b) {
    a == b | (is.na(a) & is.na(b))
...
cat("\nNumber of rows with any difference:", length(different_rows), "\n")

# 7. Optional: inspect differences in long format
diff_long <- do.call(rbind, lapply(names(old_shared), function(col) {
  idx <- which(!(new_shared[[col]] == old_shared[[col]] |
                   (is.na(new_shared[[col]]) & is.na(old_shared[[col]]))))
  ...
  data.frame(
    row = idx,
    mutation = if ("mutation" %in% names(old_shared)) old_shared$mutation[idx] else NA,
    column = col,
    old_value = old_shared[[col]][idx],
    new_value = new_shared[[col]][idx],
    stringsAsFactors = FALSE
  )
}))

cat("\nFirst differences:\n")
print(head(diff_long, 50))
